...
Donde,
\begin{itemize}
    \item $y_{it}$ representa el outcome (total de accidentes o tasa) observado en la celda $i$ durante el mes $t$
    \item $treat_i$ es una variable indicadora para las celdas que tienen un radar de velocidad. Es igual a $1$ si la celda está tratada
    \item $post_t$ es una varialbe indicadora igual a $1$ para los meses posteriores al inicio del programa (abril de 2019)
    \item $treat\_post_{it}$ es el término de interacción que captura la exposición al tratamiento de las celdas. 
...
El supuesto de identificación fundamental para esta estrategia está en que, una vez condicionado el análisis a las unidades emparejadas por el \textit{propensity score} y controlando por las características geográficas, viales y de movilidad de cada una de las areas, la ubicación de los radares de velocidad puede considerarse quasi aleatoria y no depende más de los factores que podrían influir en la siniestralidad de cada unidad. Para la estrategia del matching, adicionalmente al supuesto de independiencia condicional, se requiere la existencia del soporte común. Como se demostró en las pruebas de balance, este supuesto se satisface, dejando fuera aquellas áreas para las que no existieran suficientes unidades de control para elegir un buen contrafactual. 
